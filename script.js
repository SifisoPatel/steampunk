window.addEventListener('load', function () {
    ...
    class Enemy { //handles the enemy objects --- various types of enemies means that they will all inherit from the first enemy class 
        constructor(game) {
            this.game = game;
    ...
    class Angler1 extends Enemy {
        constructor(game) {
            ...
            this.width = 228;
            this.height = 169;
            this.y = Math.random() * (this.game.height * 0.95 - this.height); //offsetting the y position of the enemy based on individual sprite sizes
    ...
    let lastTime = 0; //used to hold timestamp from last loop for ammo regen timer
    // animation loop
    function animate(timeStamp) {
        const deltaTime = timeStamp - lastTime;
        lastTime = timeStamp;
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        game.update(deltaTime); //Periodic events will happen when they are meant to happen regardless of the processing power of the machine running the code
        game.draw(ctx);
